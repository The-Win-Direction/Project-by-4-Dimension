...
const CustomLogViewer = () => {
  ...
  return (
    <div className="p-8">
      <h2 className="text-xl font-semibold mb-6 text-green-700">📝 Custom Logs</h2>

      {loading ? (
        <p>Loading logs...</p>
      ) : logs.length === 0 ? (
        <p className="text-gray-600">No custom logs available yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {logs.map((log, i) => (
            <div key={i} className="border rounded-lg shadow p-4 bg-white">
              <p className="font-semibold text-gray-800">Title: <span className="text-gray-600">{log.title}</span></p>
              <p className="text-sm text-gray-700">Notes: {log.notes}</p>
              {log.imageUrl && (
                <img
                  src={log.imageUrl}
                  alt="Custom log"
